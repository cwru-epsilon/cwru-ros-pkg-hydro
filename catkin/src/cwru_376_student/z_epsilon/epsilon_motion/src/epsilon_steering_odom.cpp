//epsilon_steering_algorithm.cpp:
//aab119, Mar 2015
...
bool SteeringController::epsilon_steering_algorithm() {
    ...
    if (lateral_err <= -0.0001 || lateral_err >= 0.0001) { //if positive, then desired state is to the left of odom
        ...
        controller_omega = controller_omega + sgn(lateral_err)*0.02; 
    ...
    if (trip_dist_err <= -0.001 || trip_dist_err >= 0.001) { //if positive, then we are behind schedule
        ...
        controller_speed = controller_speed + sgn(trip_dist_err)*0.02;
