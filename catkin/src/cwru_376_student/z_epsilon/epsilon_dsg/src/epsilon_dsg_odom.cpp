...
#include "epsilon_des_state_generator_odom.h"
int ans;
...
DesStateGenerator::DesStateGenerator(ros::NodeHandle* nodehandle) : nh_(*nodehandle) { // constructor
    ...
    //last_map_pose_rcvd_ = odom_to_map_pose(odom_pose_stamped_); 
}
...
double DesStateGenerator::compute_speed_profile() {
    ...
    else if (current_seg_length_to_go_ <= dist_decel+LENGTH_TOL) { //It seems like dist_decel IS NOT ENOUGH...
